...
import com.code.generator.exception.FileArchiveException;
...
public class FileArchiveService {
	...
	public void archiveFiles(final String customerId,
		...
		this.createArchive(customerId, sourceDirectoryName, zipDestinationName);
	}
	...
	private String getFileName(final String customerId, String filePath) {
		String name = filePath.substring((CODEGENERATION_DIR+"/"+customerId).length() + 1, filePath.length());
		return name;
	}
		...
		try {
			...
			for (File file : fileList) {
				...
				if (file.isDirectory()) {
					...
					ZipEntry ze = new ZipEntry(getFileName(customerId, file.toString()) + "/");
