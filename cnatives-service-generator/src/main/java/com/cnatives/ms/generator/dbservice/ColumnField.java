...
public class ColumnField {
	...
	public ColumnField(String datatypeClassName) {
...
//	}
	...
	private String computedDatatype(final String datatype) {
		...
		if(null == this.datatypeClassName) {
			computedDataType = datatype;
		}
		else if(this.datatypeClassName.toLowerCase().equals(Constants.STRING)) {
			computedDataType = Constants.VARCHAR;
		}
		else if(this.datatypeClassName.toLowerCase().equals(Constants.CHAR)) {
			computedDataType = Constants.CHAR;;
		}
		else if(this.datatypeClassName.toLowerCase().equals(Constants.SHORT)) {
			computedDataType = Constants.INT;
		}
		else if(this.datatypeClassName.toLowerCase().equals(Constants.INTEGER)) {
			computedDataType = Constants.INT;
		}
		else if(this.datatypeClassName.toLowerCase().equals(Constants.LONG)) {
			computedDataType = Constants.INT;
		}
		else if(this.datatypeClassName.toLowerCase().equals(Constants.FLOAT)) {
			computedDataType = Constants.FLOAT8;
		}
		else if(this.datatypeClassName.toLowerCase().equals(Constants.DOUBLE)) {
			computedDataType = Constants.FLOAT8;
		}
		else if(this.datatypeClassName.toLowerCase().equals(Constants.BOOLEAN)) {
			computedDataType = Constants.BOOLEAN;
		}
		else if(this.datatypeClassName.toLowerCase().equals(Constants.DATE) || 
				this.datatypeClassName.toLowerCase().equals(Constants.TIMESTAMP)) {
			computedDataType = Constants.TIMESTAMP;
		}
		else if(this.datatypeClassName.toLowerCase().equals(Constants.BYTE)) {
	...
	public String getColumnSscript(String databaseType) {
		...
		if(databaseType.equalsIgnoreCase("mysql")) {
			scriptGenerator = new MysqlScriptGenerator(this);
		}
//		else {
